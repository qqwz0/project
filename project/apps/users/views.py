...
def microsoft_register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request, "auth/register.html", {"form": form, "errors": form.errors}
            )
    else:
    ...
    if request.method == "GET":
        ...
        params = {
            ...
            "redirect": request.GET.get(
                "redirect", ""
            ),  # Preserve the redirect URL if provided
        }
        authorization_url = f"{MICROSOFT_AUTH_URL}?{urlencode(params)}"
...
def microsoft_callback(request):
    ...
    logger.error(
        f"[CSRF DEBUG] session.csrf_state={request.session.get('csrf_state')}, state={state}"
    )
    ...
    try:
        ...
        if received_csrf != request.session.get("csrf_state"):
            logger.error(
                "CSRF check failed. Received: %s, Expected: %s",
                received_csrf,
                request.session.get("csrf_state"),
            )
...
def handle_login_callback(request, code):
    ...
    try:
        ...
        try:
            ...
            messages.error(
                request, "Користувача з цією електронною поштою не знайдено."
            )
            return redirect("login")
...
def profile(request: HttpRequest, user_id=None):
    ...
    if user_profile.role == "Викладач":
        ...
        active_requests = (
            ...
            .prefetch_related("student_themes", "files")
            .filter(teacher_id__teacher_id=user_profile, request_status="Активний")
        )

        # Get files for active requests
        active_request_files = {}
        for req in active_requests:
            files = RequestFile.objects.filter(request=req).select_related(
                "uploaded_by"
            )
        ...
        rejected_requests = (
            ...
            .prefetch_related("student_themes", "files")
            .filter(teacher_id__teacher_id=user_profile, request_status="Відхилено")
        )
        ...
        context.update(
            {
                ...
                .prefetch_related("student_themes", "files")
                .filter(teacher_id__teacher_id=user_profile, request_status="Очікує"),
                ...
                .prefetch_related("student_themes", "files")
                .filter(
                    teacher_id__teacher_id=user_profile, request_status="Завершено"
                ),
        ...
        all_requests = (
            ...
            .filter(student_id=user_profile)
        )
        ...
        context.update(
            {
                ...
                "has_rejected": all_requests.filter(
...
def reject_request(request, request_id):
    ...
    if request.method == "POST":
        try:
            ...
            logger.debug(
                f"Reject Request ID: {req.id}, Student: {req.student_id}, Teacher: {req.teacher_id}"
            )
            ...
            if req.teacher_id.teacher_id == request.user:
                ...
                req.rejected_reason = (
                    reason if reason else "Викладач не вказав причину відмови"
                )
                req.save()
                ...
                logger.warning(
                    f"User {request.user.id} attempted to reject request {request_id} belonging to teacher {req.teacher_id.teacher_id.id}"
                )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                )  # Redirect to crop profile picture view
            except Exception as e:
                logger.error(
                    f"Error saving profile picture for user {request.user.id}: {str(e)}"
                )
    ...
    if request.method == "POST":
        ...
        if crop_form.is_valid():
            ...
            try:
                ...
                logger.exception(
                    f"Error during forced Cloudinary upload for user {request.user.id}: {str(e)}"
                )
                return JsonResponse(
                    {"success": False, "error": "Error processing the image."}
                )
        else:
            logger.warning(
                f"Crop form is invalid for user {request.user.id}: {crop_form.errors}"
            )
...
def teacher_profile_edit(request):
    ...
    if request.method == "POST":
        form = TeacherProfileForm(
            request.POST, instance=teacher_profile, user=request.user
        )
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    if new_themes_data and new_themes_data != "[]":
                        try:
                            ...
                            for theme in existing_themes:
                                if theme.theme not in new_theme_texts:
                                    ...
                                    is_in_use = Request.objects.filter(
                                        Q(teacher_theme=theme)
                                        | Q(approved_student_theme__theme=theme.theme),
                                    ...
                                    if is_in_use:
                                        ...
                                        logger.warning(
                                            f"User {request.user.id} failed to delete theme '{theme.theme}' as it is in use."
                                        )
                                        return JsonResponse(
                                            {
                                                "success": False,
                            ...
                            for theme_data in new_themes:
                                ...
                                if theme_text:
                                    theme_obj, created = (
                                        TeacherTheme.objects.update_or_create(
                                            teacher_id=teacher_profile,
                                            theme=theme_text,
                                            defaults={
                                                "theme_description": theme_data.get(
                                                    "description", ""
                                                ),
                            ...
                            logger.error(
                                f"JSON Decode Error: {str(e)} - Data: {new_themes_data}"
                            )
                            if (
                    ...
                    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                        return JsonResponse(
                            {"success": True, "message": "Профіль успішно оновлено"}
                        )
...
def teacher_requests(request):
    return render(
        request, "profile/requests.html", {"message": "У вас немає нових запитів."}
    )
...
def complete_request(request, request_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": "Сталася помилка при обробці запиту"}
        )
        ...
        if tab_name == "active":
            if request.user.role == "Студент":
                active_requests = Request.objects.filter(
                    student_id=request.user, request_status="Активний"
                ).select_related("teacher_id__teacher_id", "teacher_theme", "slot")

                # Створюємо словник для файлів
                active_request_files = {}
                for req in active_requests:
                    files = RequestFile.objects.filter(request=req).order_by(
                        "-uploaded_at"
                    )
                ...
                active_requests = Request.objects.filter(
                    teacher_id__teacher_id=request.user, request_status="Активний"
                )
...
def restore_request(request, request_id):
    ...
    if request.method == "POST":
        try:
            ...
            # Add debugging logs
            logger.debug(
                f"Restore Request ID: {req.id}, Student: {req.student_id}, Teacher: {req.teacher_id}"
            )
            logger.debug(f"Current user: {request.user.id} - {request.user}")
            logger.debug(
                f"Teacher user: {req.teacher_id.teacher_id.id} - {req.teacher_id.teacher_id}"
            ...
            if req.teacher_id.teacher_id == request.user:
                ...
                logger.warning(
                    f"User {request.user.id} attempted to restore request {request_id} belonging to teacher {req.teacher_id.teacher_id.id}"
                )
...
def request_details_for_approve(request, request_id):
    ...
    try:
        ...
        themes = [
            {"id": f"teacher_{t.id}", "title": f"Тема викладача: {t.theme}"}
            for t in teacher_themes
        ] + [
            {"id": f"student_{t.id}", "title": f"Тема студента: {t.theme}"}
            for t in student_themes
        ]
...
def approve_request_with_theme(request, request_id):
    ...
    try:
        req = (
            ...
            .select_related("teacher_id", "teacher_theme")
            .get(id=request_id, request_status="Очікує")
        )
        ...
        if str(theme_id).startswith("teacher_"):
            ...
            if theme.is_occupied and theme != original_teacher_theme:
                return JsonResponse(
                    {"error": "Ця тема вже зайнята іншим студентом."}, status=400
                )
            new_teacher_theme = theme

            req.teacher_theme = theme
            req.approved_student_theme = (
                None  # Явно очищаємо, якщо раніше була студентська
            )
            ...
            req.approved_student_theme = (
                theme  # Зберігаємо посилання на студентську тему
            )
        ...
        logger.info(
            f"Request {req.id} approved with theme. {cancelled_count} other pending requests cancelled for student {student_id.id}"
        )

        req.save()
        return JsonResponse({"success": True, "cancelled_count": cancelled_count})
...
def edit_request_theme(request, request_id):
    ...
    try:
        ...
        logger.info(
            f"Found request: {req.id}, teacher: {req.teacher_id.teacher_id}, current user: {request.user}"
        )

        # Check if user is the teacher of this request
        if request.user.role != "Викладач" or req.teacher_id.teacher_id != request.user:
            logger.warning(
                f"Permission denied: user {request.user.id} trying to edit theme for request {request_id}"
            )
            return JsonResponse(
        ...
        return JsonResponse(
            {"error": "Запит не знайдено або не є активним"}, status=404
        )
...
def edit_student_request(request, request_id):
    ...
    current_student_theme_ids = {
        int(tid) for tid in request.POST.getlist("themes") if tid.isdigit()
    }
    ...
    if total_student_themes > 3:
        return JsonResponse(
            {
                "success": False,
                "error": "Ви можете запропонувати не більше 3 власних тем.",
            },
            status=400,
        )
    ...
    if total_student_themes == 0 and not new_teacher_theme_id:
        return JsonResponse(
            {
                "success": False,
                "error": "Необхідно обрати тему викладача або запропонувати власну.",
            },
            status=400,
        )
    ...
    StudentTheme.objects.filter(request=req).exclude(
        id__in=current_student_theme_ids
    ).delete()

    for theme_text in new_themes:
        StudentTheme.objects.create(
            request=req, student_id=request.user, theme=theme_text
        )
    ...
    if new_teacher_theme_id and new_teacher_theme_id.isdigit():
        try:
            ...
            return JsonResponse(
                {"success": False, "error": "Обрану тему викладача не знайдено."},
                status=400,
            )
    ...
    if old_teacher_theme != new_teacher_theme:
        ...
        if new_teacher_theme:
            if new_teacher_theme.is_occupied:
                return JsonResponse(
                    {"success": False, "error": "Ця тема вже зайнята іншим студентом."},
                    status=400,
                )
            new_teacher_theme.is_occupied = True
            new_teacher_theme.save()
...
def activate_teacher_theme(request, theme_id):
    ...
    try:
        ...
        if (
            request.user.role != "Викладач"
            or theme.teacher_id.teacher_id != request.user
        ):
            return JsonResponse(
                {"error": "У вас немає прав для активації цієї теми"}, status=403
            )
        ...
        return JsonResponse({"success": True, "message": "Тему успішно активовано"})
...
def deactivate_teacher_theme(request, theme_id):
    ...
    try:
        ...
        if (
            ...
            return JsonResponse(
                {"error": "У вас немає прав для деактивації цієї теми"}, status=403
            )
        ...
        return JsonResponse({"success": True, "message": "Тему успішно деактивовано"})
        ...
        return JsonResponse(
            {"error": "Сталася помилка при деактивації теми"}, status=500
        )
...
def delete_teacher_theme(request, theme_id):
    ...
    try:
        ...
        if (
            ...
            return JsonResponse(
                {"error": "У вас немає прав для видалення цієї теми"}, status=403
            )
        ...
        return JsonResponse(
            {"success": True, "message": f'Тему "{theme_name}" успішно видалено'}
        )
...
def attach_theme_to_streams(request, theme_id):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Сталася помилка при прикріпленні теми до потоків"}, status=500
        )
...
def create_teacher_theme(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Помилка при створенні теми: {str(e)}"}, status=500
        )
...
def update_teacher_theme(request, theme_id):
    ...
    try:
        ...
        existing_theme = (
            TeacherTheme.objects.filter(teacher_id=theme.teacher_id, theme=theme_name)
            .exclude(id=theme_id)
            .first()
        )
        ...
        if existing_theme:
            return JsonResponse(
                {"error": "У вас вже є тема з такою назвою"}, status=400
            )
...
def delete_theme(request, theme_id):
    ...
    try:
        active_requests = Request.objects.filter(
            teacher_theme=theme, request_status__in=["Очікує", "Активний"]
        ).exists()
...
def _group_teacher_themes_by_stream(teacher_profile):
    ...
    themes = (
        TeacherTheme.objects.filter(
            teacher_id=teacher_profile, is_deleted=False, is_active=True
        )
        .exclude(theme_description="(Запропоновано студентом)")
