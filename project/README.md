# SciAvisor / Advisor Finder (Django-проєкт)

Цей README описує, як влаштований і як експлуатується ваш Django-проєкт, які сценарії підтримує застосунок, та які проблеми вже зафіксовані в логах/коді з пропонованим планом виправлень.
...
## Примітка про актуальність

Цей README базується на поточному коді в репозиторії та на наявних логах. Якщо ви запускаєте систему з іншою версією коду або іншою схемою БД, частина симптомів може відрізнятися. Тому в плані виправлень передбачені кроки “звірити модель і колонки”.
