    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      document.body.addEventListener('click', function(e){
        if(e.target && e.target.classList.contains('reply-btn')){
          ...
          if(!container) {
            console.log('No file-comments container found');
            return;
          }
      ...
      document.body.addEventListener('submit', async function(e){
        ...
        console.log('Reply form submit detected:', form);
        e.preventDefault();
        ...
        try {
          ...
          if (data && data.success) {
            ...
            const list = form.closest('.file-comments').querySelector('.comments-list');
            if (list) {
              ...
              if (parentId) {
                ...
                if (parentComment) {
                  const parentAuthor = parentComment.querySelector('.comment-author').textContent;
                  const parentDate = parentComment.querySelector('.comment-date').textContent;
                  const parentText = parentComment.querySelector('.comment-text').textContent;
                  const shortText = parentText.length > 100 ? parentText.substring(0, 100) + '...' : parentText;
                  ...
                  quotedHtml = `
                    <div class="quoted-comment">
                      ...
                      <p class="quoted-text">${shortText.replace(/</g,'&lt;')}</p>
                    </div>
                  `;
                }
              }
              ...
              item.innerHTML = `
                ...
                <p class="comment-text">${(data.text || '').replace(/</g,'&lt;')}</p>
              `;
