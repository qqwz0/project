...
(function () {
  ...
  function getTeacherProfileSteps() {
    ...
    steps.push(safeStepPos('#active-section .complete-button', 'Завершити роботу: дозволяє виставити оцінку та перемістити роботу в архів.', 'top'));
    steps.push(safeStepPos('#active-section .cancel-button', 'Скасувати роботу: у тому випадку, якщо прийняли запит помилково.', 'top'));
    // 4. Архів
  ...
  function getTeacherEditSteps() {
    ...
    steps.push(safeStep('.themes-section h3', 'Управління темами: Секція для управління Вашими темами.'));
    // 5) Додати нову тему
    steps.push(safeStep('.add-theme-btn, .add-theme', 'Додати нову тему: Створіть тему, вказавши назву, опис та оберіть відповідні потоки, для яких ця тема буде доступною.'));
    // 6) Дії з темою (об'єднані)
    steps.push(safeStep('.themes-grid .theme-card:first-child .theme-actions', 'Дії з темою: Редагувати (змінити назву, опис, потоки), Активувати/Деактивувати (зробити доступною/недоступною для запитів), Видалити (повне видалення теми).'));
    // 7) Зберегти
    steps.push(safeStep('.save-button', 'Зберегти: Збережіть щойно внесені зміни профілю.'));
    // 8) Завершити
    steps.push(safeStep('.back-button', 'Завершити: Повернутися до сторінки профілю після збереження змін.'));
    return steps.filter(Boolean);
  }
  ...
  function startTour(context) {
    ...
    var customSteps = steps.map(function(step) {
